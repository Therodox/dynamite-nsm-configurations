...
def get_dev_versions():
    ...
    s3 = boto3.client('s3', aws_access_key_id=config['AWS']['aws_access_key_id'],
                      aws_secret_access_key=config['AWS']['aws_secret_access_key'])
    for key in s3.list_objects(Bucket='dynamite-config-staging', Prefix='dynamite-dev')['Contents']:
        key = key['Key'].split('/')
        if len(key) != 3:
            continue
        try:
...
def upload_to_dev(default_configs_directory, mirrors_directory, version, overwrite=False):
    existing_versions = get_pro_versions()
    if not overwrite:
        if float(version) in existing_versions:
            print('A version of this number already exists.')
    ...
    Configurations can now be downloaded from:
     - https://dynamite-config-staging.s3-us-west-2.amazonaws.com/dynamite-dev/latest/default_configs.tar.gz
     - https://dynamite-config-staging.s3-us-west-2.amazonaws.com/dynamite-dev/latest/mirrors.tar.gz

     AND

     - https://dynamite-config-staging.s3-us-west-2.amazonaws.com/dynamite-dev/{}/default_configs.tar.gz
     - https://dynamite-config-staging.s3-us-west-2.amazonaws.com/dynamite-dev/{}/mirrors.tar.gz
    '''.format(version, version))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dynamite-pro', default=False, dest='dynamite_pro', action='store_true',
                        help='Upload to Dynamite Pro staging environment instead of public/dev.')
    parser.add_argument('--dynamite-dev', default=False, dest='dynamite_dev', action='store_true',
                        help='Upload to Dynamite Dev environment instead of public/pro.')
